        ...
        <section class="section-tabs"><el-tabs v-model="tab"><el-tab-pane label="研究指标与曲线" name="research" /><el-tab-pane label="配置 / 产物" name="evidence" /></el-tabs>
          <div v-if="tab === 'research'" class="research-stack">
            <section v-if="selectedMatrixRow" class="research-overview" aria-label="当前研究单元摘要">
              <div class="overview-title"><div><span class="eyebrow">DECISION SNAPSHOT</span><h2>证据摘要</h2><p>{{ selectedMatrixRow.factor_ref }} · {{ selectedMatrixRow.horizon }}D</p></div><span class="decision-state" :data-mark="selectedMatrixRow.decision?.mark ?? 'UNREVIEWED'">{{ decisionLabel(selectedMatrixRow.decision?.mark) }}</span></div>
              ...
              <div class="decision-workbench">
                <div><span class="eyebrow">HUMAN DECISION</span><strong>记录人工结论</strong><small>结论只绑定当前四维研究单元，不会改写统计证据。</small></div>
            ...
            <section v-for="group in curveGroups" :key="group.title" class="curve-section">
              ...
              <summary class="panel-heading"><div><h2>{{ group.title }}</h2><p>按当前研究单元读取可信产物。</p></div><span>{{ group.curves.length }} 张图 <b>展开</b></span></summary>
