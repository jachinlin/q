...
const artifactType = computed(() => ({ signals: 'signals', portfolio: 'holdings', execution: executionArtifact.value, performance: performanceArtifact.value, attribution: attributionArtifact.value } as Record<string, string>)[tab.value])
...
const artifact = useQuery({
  queryKey: ['run-artifact', selectedRunId, artifactType, artifactPage],
...
const quality = useQuery({
  ...
  enabled: computed(() => Boolean(selectedRun.value?.manifest_hash)),
})
...
const artifactRows = computed<RunArtifactRow[]>(() => rawArtifactRows.value)
...
const coreMetrics = computed(() => STRATEGY_CORE.map(([name, label]) => displayMetric(name, label)))
...
const chartOption = computed(() => {
  ...
  if (tab.value === 'performance' && artifactType.value === 'performance') return {
  ...
  if (tab.value === 'performance' && ['monthly_returns', 'annual_returns'].includes(artifactType.value)) return {
...
watch([artifactType, selectedRunId], () => { artifactPage.value = 1 })
</script>
...
<template>
  <div class="page-stack">
    ...
    <template v-else-if="detail.data.value">
      <section class="panel detail-head"><div><span class="eyebrow">STRATEGY BACKTEST</span><h2>{{ detail.data.value.experiment.definition.name }}</h2><p>{{ detail.data.value.experiment.definition.description }}</p></div><div class="head-status"><StatusBadge v-if="selectedRun" :status="selectedRun.status" /><strong>{{ detail.data.value.runs.length }}</strong><small> RUNS</small><el-button type="danger" plain :disabled="hasActiveRuns" :loading="removeExperiment.isPending.value" @click="deleteExperiment">删除实验</el-button></div></section>
      ...
      <section class="panel">
        <el-tabs v-model="tab"><el-tab-pane label="协议/配置" name="protocol" /><el-tab-pane label="信号" name="signals" /><el-tab-pane label="持仓" name="portfolio" /><el-tab-pane label="成本/执行" name="execution" /><el-tab-pane label="绩效时序" name="performance" /><el-tab-pane label="归因" name="attribution" /><el-tab-pane label="产物" name="artifacts" /><el-tab-pane label="Run 比较" name="compare" /><el-tab-pane label="新增 Run" name="new-run" /></el-tabs>
        ...
        <template v-else>
          <div v-if="tab === 'execution'" class="artifact-switch"><el-radio-group v-model="executionArtifact" size="small"><el-radio-button value="orders">订单</el-radio-button><el-radio-button value="fills">成交/拒绝</el-radio-button><el-radio-button value="costs">成本</el-radio-button><el-radio-button value="execution_summary">汇总</el-radio-button></el-radio-group></div>
          <div v-if="tab === 'performance'" class="artifact-switch"><el-radio-group v-model="performanceArtifact" size="small"><el-radio-button value="performance">净值/回撤</el-radio-button><el-radio-button value="monthly_returns">月度收益</el-radio-button><el-radio-button value="annual_returns">年度收益</el-radio-button></el-radio-group></div>
          <div v-if="tab === 'attribution'" class="artifact-switch"><el-radio-group v-model="attributionArtifact" size="small"><el-radio-button value="attribution">收益归因</el-radio-button><el-radio-button value="exposure_summary">仓位暴露</el-radio-button></el-radio-group></div>
