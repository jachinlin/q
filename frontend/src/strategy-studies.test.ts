...
describe('strategy study list', () => {
  ...
  it('shows a single study lifecycle and deletes a terminal study', async () => {
    ...
    const router = createRouter({ history: createMemoryHistory(), routes: [{ path: '/strategy-studies', component: StrategyStudiesView }, { path: '/strategy-studies/new', component: { template: '<div />' } }, { path: '/strategy-studies/:strategyStudyId', component: { template: '<div />' } }] })
