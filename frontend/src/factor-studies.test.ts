...
describe('independent factor study dashboard', () => {
  ...
  it('deep-links global selectors, saves matrix decisions and maps IC chart data', async () => {
    ...
    expect(wrapper.find('.research-overview').exists()).toBe(true)
    expect(wrapper.get('.research-overview').text()).toContain('摘要')
    expect(wrapper.get('.research-overview').text()).not.toContain('证据摘要')
    expect(wrapper.get('.decision-workbench').text()).toBe('结论CandidateDiscarded清除')
    expect(wrapper.text()).not.toContain('按当前研究单元读取可信产物')
    expect(wrapper.get('.summary-detail').attributes('open')).toBeUndefined()
    expect(wrapper.findAll('.curve-disclosure')).toHaveLength(3)
    expect(wrapper.findAll('.curve-disclosure')[0].attributes('open')).toBeDefined()
