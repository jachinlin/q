...
def test_constructor_stably_orders_score_ties_and_limits_positions() -> None:
    candidates = _candidates(
        [
            _row("SSE:600002", 10.0),
            _row("SSE:600001", 10.0),
            _row("SZSE:000001", 9.0),
        ]
    )

    result = PortfolioConstructor().construct(
        candidates,
        _constraints(max_positions=2, max_industry_weight=1.0),
        _SIGNAL_DATE,
        _EXECUTE_DATE,
    )
...
def test_constructor_caps_single_names_and_leaves_unallocated_cash() -> None:
    result = PortfolioConstructor().construct(
        _candidates([_row("SSE:600001", 10.0)]),
...
def test_constructor_scales_an_overweight_industry_without_redistributing_cash() -> (
    None
):
    result = PortfolioConstructor().construct(
        _candidates(
            [
                _row("SSE:600001", 3.0, "BANK"),
                _row("SSE:600002", 2.0, "BANK"),
                _row("SZSE:000001", 1.0, "TECH"),
            ]
        ),
        _constraints(max_industry_weight=0.4),
        _SIGNAL_DATE,
        _EXECUTE_DATE,
    )

    assert [position.target_weight for position in result.positions] == pytest.approx(
        [0.2, 0.2, 1 / 3]
    )
    assert result.cash_weight == pytest.approx(4 / 15)
...
def test_constructor_rejects_insufficient_liquid_candidates() -> None:
    with pytest.raises(ConstraintViolation) as caught:
        PortfolioConstructor().construct(
            _candidates([_row("SSE:600001", 4.0, adv_amount=99.0)]),
...
def test_constructor_rejects_a_liquid_scored_candidate_without_an_industry(
    industry: str | None,
) -> None:
    with pytest.raises(ValueError, match="industry"):
        PortfolioConstructor().construct(
            _candidates([_row("SSE:600001", 4.0, industry=industry)]),
            _constraints(),
            _SIGNAL_DATE,
            _EXECUTE_DATE,
        )
...
def test_constructor_rejects_turnover_above_constraint() -> None:
    with pytest.raises(ConstraintViolation) as caught:
        PortfolioConstructor().construct(
            _candidates(
                [
                    _row("SSE:600001", None, current_weight=1.0),
                    _row("SZSE:000001", 3.0),
                ]
            ),
...
def test_constructor_rejects_non_forward_execution_date() -> None:
    with pytest.raises(ValueError, match="execute_date"):
        PortfolioConstructor().construct(
            _candidates([_row("SSE:600001", 1.0)]),
            _constraints(),
            _SIGNAL_DATE,
            _SIGNAL_DATE,
        )
...
@pytest.mark.parametrize(
    ("mutate", "message"),
    [
        (lambda frame: frame.drop("industry"), "industry"),
        (
...
def test_constructor_fails_closed_for_invalid_candidate_contract(
    ...
    base = _candidates([_row("SSE:600001", 1.0)])
...
@given(
    ...
    max_industry_weight=st.floats(
        min_value=0.01, max_value=1.0, allow_nan=False, allow_infinity=False
    ),
)
def test_successful_portfolios_keep_weights_nonnegative_and_bounded(
    scores: list[float], max_position_weight: float, max_industry_weight: float
) -> None:
    rows = [
        _row(f"SSE:{600000 + index:06d}", score, f"INDUSTRY_{index % 2}")
