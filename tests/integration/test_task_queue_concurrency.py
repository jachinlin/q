...
from quant_core.tasks.queue import TaskQueue, TaskQueueBusy
...
def _run_operation(
    ...
    try:
        ...
        if kind == "claim":
            result: object = queue.claim(
                cast(str, arguments["worker_id"]), NOW
            )
        elif kind == "enqueue":
            result = queue.enqueue(
                ...
                idempotency_key=cast(
                    str | None, arguments.get("idempotency_key")
                ),
            )
        elif kind == "sleep":
...
def test_concurrent_claim_rounds_preserve_priority_without_duplicates(
    ...
    try:
        ...
        claimed_ids = [
            result.id
            for result in (*first_round, *second_round)
