    ...
    with engine.begin() as connection:
        connection.execute(
            text(
                ...
                "id, strategy_id, config_json, config_hash, data_hash, source_tree_hash, "
                "git_commit_hash, lockfile_hash, rulebook_hash, fingerprint, "
                "status, research_mark, created_at"
                ") VALUES ("
                ":id, 'momentum', '{}', :hash, :hash, :hash, NULL, :hash, :hash, "
                ":hash, 'CREATED', 'UNREVIEWED', :now"
                ")"
            ),
...
def test_final_schema_indexes_encode_claim_order_and_active_idempotency(
    tmp_path: Path,
) -> None:
...
def _queue_model(name: str) -> type[Any]:
    models = importlib.import_module("quant_research.tasks.models")
    model = getattr(models, name, None)
    assert model is not None, f"quant_research.tasks.models.{name} is missing"
    return model
...
def _queue(
    ...
    *,
    ...
    assert queue_type is not None, (
        "quant_research.infrastructure.persistence.task_queue.TaskQueue is missing"
    )
    return queue_type(
        engine,
...
def test_bind_log_path_derives_one_idempotent_path_from_the_trusted_root(
    engine: Engine,
    tmp_path: Path,
) -> None:
    log_root = tmp_path / "task-logs"
    unconfigured = _queue(engine)
    queue = _queue(engine, task_log_root=log_root)
...
def test_enqueue_without_key_creates_fresh_canonical_standalone_tasks(
    ...
    assert rows == [
        (
            ...
            "QUEUED",
            7,
            '{"completed":0,"context":{},"message":"","stage":"queued","total":0}',
            NOW_TEXT,
        ),
        (
            None,
...
def test_active_idempotency_returns_same_task_and_audits_canonical_hit(
    ...
    with engine.connect() as connection:
        ...
        assert (
            connection.execute(text("SELECT count(*) FROM task_attempt")).scalar_one()
            == 0
        )
...
def test_active_idempotency_rejects_payload_mismatch_without_mutation(
    ...
    queue = _queue(engine)
    task_id = queue.enqueue("BACKTEST", {"window": 20}, 0, idempotency_key="request-1")

    with pytest.raises(QuantError) as captured:
        queue.enqueue("BACKTEST", {"window": 60}, 0, idempotency_key="request-1")
...
def test_claim_atomically_persists_attempt_ownership_progress_and_audit(
    ...
    assert task["locked_at"] == NOW_TEXT.replace("08:00:00", "08:00:01")
...
def test_heartbeat_is_owner_fenced_and_synchronizes_task_attempt(
    ...
    heartbeat_at = datetime(2026, 7, 30, 16, 0, 10, tzinfo=timezone(timedelta(hours=8)))
    ...
    assert (
        task["heartbeat_at"] == attempt["heartbeat_at"] == ("2026-07-30T08:00:10+00:00")
    )
...
def test_orphan_scan_uses_newer_task_or_attempt_heartbeat(engine: Engine) -> None:
    ...
    with engine.begin() as connection:
        ...
        connection.execute(
            text(
                "UPDATE task_attempt SET heartbeat_at = :fresh WHERE task_id = :task_id"
            ),
