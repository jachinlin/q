...
class ArtifactEntry:
    """记录一个回测产物文件的相对路径、哈希、字节数、Schema 和行数。
...
class ExperimentArtifactEntry:
    ...
    def __post_init__(self) -> None:
        _ArtifactsSupport._validate_safe_file_path(self.path)
        if type(self.size_bytes) is not int or self.size_bytes < 0:
            raise ValueError("size_bytes must be a nonnegative integer")
        if not _ArtifactsSupport._is_sha256(self.sha256):
            raise ValueError("sha256 must be a lowercase SHA-256 hex digest")
    ...
    def manifest_value(self) -> dict[str, JsonValue]:
        ...
        异常：
            无。
        Return the exact experiment-index representation.
        """
...
@dataclass(frozen=True, slots=True)
class BacktestArtifactRecovery:
    """表示从完整中间回测 bundle 深度校验并恢复出的状态。

    入参：
        artifact_dir：中间回测 bundle 目录。
        manifest_path：已验证的 manifest 路径。
        sessions_completed：已完成交易日数量。
        final_snapshot：由最后一日 NAV 与持仓重建的账户快照。
    返回值：
        构造并返回不可变恢复结果。
    异常：
        字段类型或路径关系无效时抛出 ``TypeError`` 或 ``ValueError``。
    """
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.artifact_dir, Path) or not isinstance(
            ...
            raise TypeError("artifact recovery paths must be Paths")
        if self.manifest_path != self.artifact_dir / "manifest.json":
            raise ValueError(
                "recovery manifest_path must be artifact_dir/manifest.json"
            )
        if type(self.sessions_completed) is not int or self.sessions_completed <= 0:
            raise ValueError("sessions_completed must be positive")
...
class ManifestContext:
    """绑定生成回测 Manifest 所需的实验、数据、策略和规则身份。
    ...
    def __post_init__(self) -> None:
        ...
        _ArtifactsSupport._validate_sha256(self.data_hash, "data_hash")
        _ArtifactsSupport._validate_sha256(self.rulebook_hash, "rulebook_hash")
        if any(
            not isinstance(value, str) or not value for value in (self.strategy_id,)
        ):
        ...
        if self.industry_input is not None:
            if not isinstance(self.industry_input, Mapping):
                raise TypeError("manifest industry_input must be a mapping or None")
    ...
    def build_manifest(
        ...
        入参：
            entries：清单条目。
            completed_sessions：完成交易会话集合。
        返回值：
            返回构建产物清单后的产物清单（``dict[str, object]``）。
        异常：
            无。
        """
        ...
        if self.industry_input is not None:
            manifest["industry_input"] = dict(self.industry_input)
        return manifest
...
class BacktestArtifactWriter:
    ...
    返回值：
        返回完成字段规范化和不变量校验的对象。
    异常：
        输入、状态或依赖结果违反契约时抛出 ``FileExistsError``、``TypeError``。
    ...
    def append_execution(self, execution: ExecutionBatch) -> None:
        ...
        for index, result in enumerate(execution.results):
            base = {
                ...
                "requested_reference_value_fen": (result.requested_reference_value_fen),
    ...
    def validate(
        ...
        入参：
            expected_sessions：参与本次处理的``expected``交易会话集合；调用方不得依赖未声明的顺序。
            context：本次调用的上下文，类型为 ``ManifestContext``。
        返回值：
            返回校验回测后的``validate``（``dict[str, ArtifactEntry]``）。
        异常：
            输入、状态或依赖结果违反契约时抛出 ``TypeError``、``ValueError``。
        """
        ...
        entries = _ArtifactsSupport._collect_entries(self._staging_dir)
        _ArtifactsSupport._validate_content(self._staging_dir, expected_sessions)
    ...
    def publish(self) -> Path:
        ...
        入参：
            无。
        返回值：
            返回校验并原子发布回测后的``publish``（``Path``）。
        异常：
            输入、状态或依赖结果违反契约时抛出 ``FileExistsError``、``RuntimeError``、``ValueError``。
        """
        ...
        entries = _ArtifactsSupport._collect_entries(self._staging_dir)
        _ArtifactsSupport._validate_content(self._staging_dir, self._expected_sessions)
        ...
        if (self._final_dir / "manifest.json").exists() or self._final_dir.exists():
            raise FileExistsError("experiment artifact directory already exists")
        staging_manifest = self._staging_dir / "manifest.json"
        _ArtifactsSupport._write_json(staging_manifest, manifest)
        raw, written = _ArtifactsSupport._read_experiment_manifest(staging_manifest)
        if raw != canonical_json_bytes(cast(JsonValue, written)) or written != manifest:
            raise ValueError("backtest manifest changed before publication")
    ...
    def _safe_diagnostic(
        ...
        try:
            _ArtifactsSupport._write_json(
                self._staging_dir / "diagnostic.json", diagnostic
            )
...
def validate_backtest_artifacts(
    artifact_dir: Path, *, context: ManifestContext
) -> BacktestArtifactRecovery:
    """深度校验中间回测 bundle；该函数作为稳定公开 API 保留在模块级。

    入参：
        artifact_dir：名称与实验 UUID 一致的中间 bundle 目录。
        context：本次实验期望的完整回测 manifest 上下文。
    返回值：
        返回包含最后账户快照的不可变恢复结果。
    异常：
        TypeError：路径或上下文类型不符合契约时抛出。
        ValueError：文件、身份、哈希、schema、行数或业务内容不匹配时抛出。
    """
    ...
    if not isinstance(context, ManifestContext):
        raise TypeError("context must be a ManifestContext")
    expected_name = f"experiment_id={context.experiment_id}"
    if artifact_dir.name != expected_name:
        raise ValueError("backtest artifact directory identity is invalid")
    ...
    if raw != canonical_json_bytes(cast(JsonValue, manifest)):
        raise ValueError("backtest manifest must be canonical UTF-8 JSON")
    entries = _ArtifactsSupport._collect_entries(artifact_dir)
    nav_dates = tuple(
    ...
    if not nav_dates or any(not isinstance(value, date) for value in nav_dates):
        raise ValueError("backtest nav timeline is invalid")
    sessions = cast(tuple[date, ...], nav_dates)
    _ArtifactsSupport._validate_content(artifact_dir, sessions)
    expected_manifest = context.build_manifest(entries, len(sessions))
    if manifest != expected_manifest:
        raise ValueError("backtest manifest identity does not match experiment")
    snapshot = _ArtifactsSupport._recover_final_snapshot(artifact_dir, sessions[-1])
    return BacktestArtifactRecovery(
        artifact_dir, manifest_path, len(sessions), snapshot
    )
...
def validate_experiment_artifacts(
    ...
    *,
    ...
    """校验实验产物集合；该函数作为稳定公开 API 或框架入口保留在模块级。

    入参：
        artifact_dir：完成原子发布后的不可变产物目录。
        resolved_config：参与本次处理的解析后配置配置；调用方不得依赖未声明的顺序。
    返回值：
        返回校验实验产物集合；该函数作为稳定公开 API 或框架入口保留在模块级后的实验产物集合（``ExperimentArtifactPublication``）。
    异常：
        输入、状态或依赖结果违反契约时抛出 ``TypeError``、``ValueError``。
    ...
    with _PartitionLock(lock_path, timeout_seconds=60.0, stale_after_seconds=0.0):
        ...
        entries = _ArtifactsSupport._validate_experiment_bundle(
            artifact_dir,
            manifest,
            resolved_config,
            marker_present=True,
...
def publish_experiment_artifacts(
    ...
    *,
    ...
    入参：
        ...
        resolved_config：参与本次处理的解析后配置配置；调用方不得依赖未声明的顺序。
    返回值：
        返回校验并原子发布实验产物集合后的实验产物集合（``ExperimentArtifactPublication``）。
    异常：
        输入、状态或依赖结果违反契约时抛出 ``FileExistsError``、``TypeError``、``ValueError``。
    ...
    artifact_root.mkdir(parents=True, exist_ok=True)
    _ArtifactsSupport._require_plain_directory(artifact_root, "artifact_root")
    staging_identity = _ArtifactsSupport._require_plain_directory(
        staging_dir, "staging_dir"
    )
    if _ArtifactsSupport._path_lexists(final_dir):
        raise FileExistsError("experiment artifact directory already exists")
    ...
    _, manifest = _ArtifactsSupport._read_experiment_manifest(manifest_path)
    ...
    extended_manifest["experiment"] = {
        "artifacts": {
            name: entries[name].manifest_value() for name in _EXPERIMENT_PAYLOAD_NAMES
        },
    }
    ...
    candidate_dir.mkdir()
    candidate_identity = _ArtifactsSupport._require_plain_directory(
        candidate_dir, "candidate_dir"
    )
    payload_committed = False
    try:
        _ArtifactsSupport._copy_bundle_no_follow(staging_dir, candidate_dir)
        _ArtifactsSupport._write_atomic_bytes(
            candidate_dir / "manifest.json", final_manifest_bytes
        )
        candidate_entries = _ArtifactsSupport._validate_private_candidate(
            candidate_dir,
            experiment_id,
            resolved_config,
        ...
        with _PartitionLock(lock_path, timeout_seconds=60.0, stale_after_seconds=0.0):
            ...
            if _ArtifactsSupport._path_lexists(final_dir):
                raise FileExistsError("experiment artifact directory already exists")
            _ArtifactsSupport._validate_private_candidate(
                candidate_dir,
                experiment_id,
                resolved_config,
                final_manifest_bytes,
            ...
            _ArtifactsSupport._require_same_directory(
                staging_dir, staging_identity, "staging"
            )
            _ArtifactsSupport._validate_experiment_file_set(
                staging_dir, marker_present=True
            )
            _ArtifactsSupport._atomic_directory_publish_no_replace(
        ...
        if payload_committed:
            _ArtifactsSupport._write_quarantine_diagnostic(candidate_parent, error)
        else:
            _ArtifactsSupport._remove_private_tree(candidate_parent, error)
        raise
    _ArtifactsSupport._remove_empty_directory(candidate_parent)
    return ExperimentArtifactPublication(
...
class _ArtifactsSupport:
    ...
    def _validate_private_candidate(
        ...
        *,
        ...
        _ArtifactsSupport._require_plain_directory(candidate_dir, "candidate_dir")
        raw, manifest = _ArtifactsSupport._read_experiment_manifest(
    ...
    def _read_expected_manifest(
        ...
        raw, manifest = _ArtifactsSupport._read_experiment_manifest(marker_path)
        if raw != expected_manifest_bytes:
    ...
    def _validate_private_payload(
        ...
        *,
        ...
        _ArtifactsSupport._require_plain_directory(payload_dir, "payload_dir")
        entries = _ArtifactsSupport._validate_experiment_bundle(
            payload_dir,
            manifest,
            resolved_config,
    ...
    def _copy_bundle_no_follow(source: Path, target: Path) -> None:
        ...
        _ArtifactsSupport._validate_experiment_file_set(source, marker_present=True)
        expected = ("manifest.json", *_EXPERIMENT_PAYLOAD_NAMES)
        for name in expected:
            _ArtifactsSupport._copy_file_no_follow(source / name, target / name)
        _ArtifactsSupport._validate_experiment_file_set(target, marker_present=True)

    @staticmethod
    def _copy_file_no_follow(source: Path, target: Path) -> None:
        before = _ArtifactsSupport._require_plain_file(source, source.name)
        nofollow = getattr(os, "O_NOFOLLOW", 0)
        binary = getattr(os, "O_BINARY", 0)
        source_fd = os.open(source, os.O_RDONLY | nofollow | binary)
        target_fd: int | None = None
        try:
            opened = _ArtifactsSupport._identity_from_stat(os.fstat(source_fd))
            if opened != before:
                raise ValueError(
                    f"experiment artifact {source.name} changed before copy"
                )
            target_fd = os.open(
                target,
            ...
            while True:
                ...
                while view:
                    ...
                    view = view[written:]
            os.fsync(target_fd)
            after = _ArtifactsSupport._identity_from_stat(os.fstat(source_fd))
            if (
                after != before
                or _ArtifactsSupport._require_plain_file(source, source.name) != before
            ):
                raise ValueError(
                    f"experiment artifact {source.name} changed during copy"
                )
            ...
            if target_fd is not None:
                os.close(target_fd)
        copied = _ArtifactsSupport._require_plain_file(target, target.name)
    ...
    def _require_plain_directory(path: Path, label: str) -> _PathIdentity:
        ...
        if _ArtifactsSupport._is_reparse(status) or not stat.S_ISDIR(status.st_mode):
            raise ValueError(f"{label} must be a plain non-reparse directory")
        return _ArtifactsSupport._identity_from_stat(status)
    ...
    def _require_plain_file(path: Path, label: str) -> _PathIdentity:
        ...
        if _ArtifactsSupport._is_reparse(status) or not stat.S_ISREG(status.st_mode):
            raise ValueError(
                f"experiment artifact {label} must be a plain regular file"
            )
        return _ArtifactsSupport._identity_from_stat(status)
    ...
    def _require_same_directory(
        ...
        actual = _ArtifactsSupport._require_plain_directory(path, label)
    ...
    def _require_same_file(path: Path, expected: _PathIdentity, label: str) -> None:
        if _ArtifactsSupport._require_plain_file(path, label) != expected:
    ...
    def _atomic_directory_publish_no_replace(source: Path, target: Path) -> None:
        ...
        if _ArtifactsSupport._path_lexists(target):
    ...
    def _quarantine_manifest(
        ...
        if not _ArtifactsSupport._path_lexists(manifest):
            return
        ...
        try:
            ...
            if error is not None:
                error.add_note(
                    f"failed to quarantine manifest marker: {quarantine_error}"
                )
    ...
    @staticmethod
    def _remove_private_tree(path: Path, error: BaseException | None = None) -> None:
        if not _ArtifactsSupport._path_lexists(path):
            return
        try:
            _ArtifactsSupport._require_plain_directory(path, "private directory")
            shutil.rmtree(path)
    ...
    def _validate_experiment_bundle(
        ...
        *,
        ...
        _ArtifactsSupport._validate_experiment_file_set(
            artifact_dir, marker_present=marker_present
        )
        raw_entries = _ArtifactsSupport._collect_entries(artifact_dir)
        nav_dates = tuple(
            pq.read_table(artifact_dir / "nav.parquet", columns=["trade_date"])
            .column("trade_date")
            .to_pylist()
        ...
        _ArtifactsSupport._validate_nav_timeline_against_manifest(
            cast(tuple[date, ...], nav_dates), manifest
        )
        _ArtifactsSupport._validate_content(
            artifact_dir, cast(tuple[date, ...], nav_dates)
        )
        raw_manifest = {
            name: manifest[name]
            for name in (
        ...
        _ArtifactsSupport._validate_manifest(raw_manifest, raw_entries)

        from quant_research.analytics.materialize import validate_published_analytics

        validate_published_analytics(
            artifact_dir, manifest, expected_experiment_id=expected_experiment_id
        )
        _ArtifactsSupport._validate_resolved_config(
            artifact_dir / "resolved_config.yaml", resolved_config
        )
        _ArtifactsSupport._validate_environment(artifact_dir / "environment.json")
        _ArtifactsSupport._validate_report(artifact_dir / "report.html")
        _ArtifactsSupport._validate_utf8_text(artifact_dir / "run.log", "run.log")
        entries = _ArtifactsSupport._collect_experiment_entries(artifact_dir)
        experiment_index = manifest.get("experiment")
        if require_experiment_index or experiment_index is not None:
            _ArtifactsSupport._validate_experiment_index(experiment_index, entries)
        return entries
    ...
    def _validate_nav_timeline_against_manifest(
        ...
        if nav_dates != tuple(sorted(nav_dates)) or len(set(nav_dates)) != len(
            nav_dates
        ):
    ...
    @staticmethod
    def _validate_experiment_file_set(
        artifact_dir: Path, *, marker_present: bool
    ) -> None:
        _ArtifactsSupport._require_plain_directory(artifact_dir, "artifact_dir")
        ...
        for path in artifact_dir.iterdir():
            _ArtifactsSupport._require_plain_file(path, path.name)
    ...
    def _collect_experiment_entries(
        ...
        for name in _EXPERIMENT_PAYLOAD_NAMES:
            ...
            _ArtifactsSupport._require_plain_file(path, name)
            if name in _EXPERIMENT_PARQUET_NAMES:
                try:
                    ...
                    raise ValueError(
                        f"experiment artifact {name} is not valid Parquet"
                    ) from error
                entries[name] = ExperimentArtifactEntry(
                    name,
                    path.stat().st_size,
                    _ArtifactsSupport._sha256(path),
                    schema.to_string(show_field_metadata=False),
                    row_count,
                )
            else:
                entries[name] = ExperimentArtifactEntry(
                    name, path.stat().st_size, _ArtifactsSupport._sha256(path)
                )
        return entries
    ...
    def _validate_experiment_index(
        ...
        for name, expected in entries.items():
            _ArtifactsSupport._validate_experiment_index_entry(
                name, indexed[name], expected
            )
    ...
    def _validate_experiment_index_entry(
        ...
        if not isinstance(path, str):
            raise TypeError(
                "experiment artifact path must be a safe relative file path"
            )
        _ArtifactsSupport._validate_safe_file_path(path)
        if path != name:
    ...
    def _validate_resolved_config(
        ...
        try:
            ...
            raise ValueError(
                "resolved_config.yaml YAML must contain finite JSON values"
            ) from error
        if actual != expected:
    ...
    def _validate_environment(path: Path) -> None:
        ...
        try:
            ...
            raise ValueError(
                "environment.json environment values must be finite"
            ) from error
        if raw != canonical:
        ...
        if (
            ...
            or relative.is_absolute()
            or ".." in relative.parts
            or not _ArtifactsSupport._is_sha256(payload.get("lockfile_hash"))
        ):
    ...
    def _validate_report(path: Path) -> None:
        text = _ArtifactsSupport._validate_utf8_text(path, "report.html")
    ...
    def _read_experiment_manifest(path: Path) -> tuple[bytes, dict[str, Any]]:
        ...
        try:
            ...
            raise ValueError(
                "experiment manifest must contain finite JSON values"
            ) from error
    ...
    def _write_atomic_bytes(path: Path, payload: bytes) -> None:
        temporary = _ArtifactsSupport._write_temporary_bytes(
    ...
    @staticmethod
    def _remove_empty_directory(path: Path, error: BaseException | None = None) -> None:
        try:
            path.rmdir()
        except OSError as cleanup_error:
            if error is not None:
                error.add_note(
                    f"failed to remove private candidate directory: {cleanup_error}"
                )
    ...
    @staticmethod
    def _recover_final_snapshot(
        artifact_dir: Path, final_date: date
    ) -> AccountSnapshot:
        nav_rows = pq.read_table(artifact_dir / "nav.parquet").to_pylist()
        final_nav = nav_rows[-1]
        if final_nav.get("trade_date") != final_date:
            raise ValueError("final NAV date does not match validated timeline")
        holding_rows = pq.read_table(artifact_dir / "holdings.parquet").to_pylist()
        positions = tuple(
            PositionSnapshot(
                InstrumentId.parse(cast(str, row["instrument_id"])),
                cast(int, row["total_quantity"]),
                cast(int, row["sellable_quantity"]),
                cast(int, row["cost_basis_fen"]),
                cast(int, row["market_value_fen"]),
            )
            for row in holding_rows
            if row["trade_date"] == final_date
        )
        return AccountSnapshot(
            final_date,
            cast(int, final_nav["cash_fen"]),
            positions,
            cast(int, final_nav["market_value_fen"]),
            cast(int, final_nav["nav_fen"]),
        )
    ...
    def _validate_manifest(
        ...
        if not required <= set(manifest) or not set(manifest) <= required | {
            "industry_input"
        }:
            raise ValueError("manifest has invalid fields")
        if "industry_input" in manifest:
            _ArtifactsSupport._validate_industry_input(manifest["industry_input"])
        if (
            type(manifest["initial_cash_fen"]) is not int
        ...
        _ArtifactsSupport._validate_sha256(manifest["data_hash"], "data_hash")
        _ArtifactsSupport._validate_sha256(manifest["rulebook_hash"], "rulebook_hash")
    ...
    def _validate_sha256(value: object, name: str) -> None:
        if not _ArtifactsSupport._is_sha256(value):
    ...
    def _validate_content(
        staging_dir: Path, expected_sessions: tuple[date, ...]
    ) -> None:
        ...
        _ArtifactsSupport._validate_holdings(holdings, nav_by_date)
        _ArtifactsSupport._validate_targets(targets)
        _ArtifactsSupport._validate_execution(fills, costs)
    ...
    def _validate_holdings(
        ...
        for row in rows:
            ...
            if previous is not None and key <= previous:
                raise ValueError(
                    "holdings must be date and canonical-ID sorted uniquely"
                )
            previous = key
    ...
    def _validate_targets(rows: list[dict[str, Any]]) -> None:
        ...
        for row in rows:
            grouped.setdefault((row["signal_date"], row["execute_date"]), []).append(
                row
            )
        for rows_for_target in grouped.values():
            ...
            positions = [
                row for row in rows_for_target if row["instrument_id"] is not None
            ]
            if [row["position_index"] for row in positions] != list(
                range(len(positions))
            ):
            ...
            if (
                cash["target_weight"] != cash["cash_weight"]
                or cash["score"] is not None
            ):
    ...
    def _validate_execution(
        ...
        for row in fills:
            trade_date = row["trade_date"]
            if not isinstance(trade_date, date) or not isinstance(
                row["result_index"], int
            ):
            ...
            if (reference_price is None) != (requested_reference_value is None):
                raise ValueError("fill reference price and value must be jointly set")
            if reference_price is not None:
                if (
                    not isinstance(reference_price, float)
                    or not isfinite(reference_price)
                    or reference_price <= 0
                    or not isinstance(requested_reference_value, int)
                    or requested_reference_value <= 0
                ):
                    raise ValueError("fill reference values are invalid")
                expected_reference_value = int(
                    (
                ...
                if requested_reference_value != expected_reference_value:
                    raise ValueError("fill requested reference value is invalid")
            if row["price"] is None:
                ...
                if row["reason_code"] in {"SUSPENDED", "NO_MARKET_DATA"}:
                    if reference_price is not None:
                        raise ValueError("unpriceable reject has a reference price")
                elif reference_price is None:
                    raise ValueError("priceable reject lacks a reference price")
                continue
            if (
