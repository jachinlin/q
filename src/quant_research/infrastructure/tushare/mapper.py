...
_RENAMES: Mapping[str, Mapping[str, str]] = {
    ...
    "index_daily": {
        "ts_code": "index_id", "pre_close": "preclose",
        "pct_chg": "pct_change", "vol": "volume",
    },
    "daily_basic": {
        "ts_code": "instrument_id", "turnover_rate_f": "turnover_rate_free_float",
        "dv_ratio": "dividend_yield", "dv_ttm": "dividend_yield_ttm",
        "total_mv": "total_market_value", "circ_mv": "circulating_market_value",
    },
    "suspend_d": {"ts_code": "instrument_id"},
    "stock_st": {
        "ts_code": "instrument_id", "type": "risk_type",
    ...
    "index_classify": {
        "index_code": "industry_index_id", "is_pub": "is_published",
        "src": "taxonomy",
    },
    "index_member_all": {
...
_DATE_FIELDS = frozenset(
    {
        "list_date", "delist_date", "found_date", "due_date", "issue_date",
        "purc_startdate", "redm_startdate", "base_date", "exp_date", "trade_date",
        "previous_trade_date", "announcement_date", "report_period", "in_date",
        "out_date",
    }
)
_PERCENT_FIELDS = frozenset(
    {
        "pct_change", "turnover_rate", "turnover_rate_free_float", "dividend_yield",
        "dividend_yield_ttm", "m_fee", "c_fee", "exp_return",
    }
)
...
class TushareMapper:
    ...
    def normalize(self, raw_partition: PublishedPartition) -> tuple[CanonicalBatch, ...]:
        """规范化分区。入参：Raw 分区。返回值：Canonical 批次。异常：端点或值非法时抛出。"""
        try:
            dataset = _ENDPOINT_DATASET[raw_partition.endpoint]
        except KeyError as error:
            raise ValueError(f"unsupported Tushare endpoint: {raw_partition.endpoint}") from error
        ...
        if dataset is DatasetKind.STOCK_FINANCIAL_INDICATOR:
            self._assign_revisions(normalized)
    ...
    def candidate_partition_keys(
        ...
        if len(start) >= 4 and len(end) >= 4:
            return tuple(f"year={year}" for year in range(int(start[:4]), int(end[:4]) + 1))
        return ("all",)
    ...
    def consolidate_partition(
        ...
        if dataset is DatasetKind.STOCK_FINANCIAL_INDICATOR:
            ...
            self._assign_revisions(rows)
            frame = pl.DataFrame(rows, schema=schema.columns, strict=False)
        return frame.unique(subset=schema.primary_key, keep="last").sort(schema.sort_key)
    ...
    def _normalize_row(
        ...
        renamed = {_RENAMES.get(endpoint, {}).get(key, key): value for key, value in row.items()}
        result: dict[str, object | None] = {}
        schema = CANONICAL_SCHEMAS[dataset]
        for field, dtype in schema.columns.items():
            if field in {"source", "available_at", "availability_source", "pit_usable", "ingested_at"}:
                continue
            result[field] = self._convert(field, dtype, renamed.get(field))
        if dataset is DatasetKind.STOCK_MASTER:
            result["board"] = self._board(str(result.get("market") or ""), str(result["instrument_id"]))
        ...
        available_at, availability_source = self._availability(partition, dataset, result)
        result.update(
            source="tushare",
    ...
    def _financial_percent_field(field: str) -> bool:
        return (
            ...
            or "_to_" in field
            or field in {"npta", "roic", "gross_margin", "cogs_of_sales", "expense_of_sales"}
        )
    ...
    def _availability(
        ...
        if dataset in {
            DatasetKind.STOCK_DAILY_BAR, DatasetKind.STOCK_ADJUSTMENT_FACTOR,
            DatasetKind.FUND_DAILY_BAR, DatasetKind.FUND_ADJUSTMENT_FACTOR,
            DatasetKind.INDEX_DAILY_BAR, DatasetKind.STOCK_DAILY_BASIC,
    ...
    @staticmethod
    def _assign_revisions(rows: list[dict[str, object | None]]) -> None:
        rows.sort(
            key=lambda item: (
        ...
        revisions: defaultdict[tuple[str, str], int] = defaultdict(int)
        for row in rows:
            key = (str(row.get("instrument_id")), str(row.get("report_period")))
