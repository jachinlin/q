...
class StrategyStudyProgressReporter:
    """将策略研究子步骤映射为稳定的四阶段任务进度。
    ...
    def __init__(self, sink: ProgressSink) -> None:
        self._sink = sink
        self._stage = StrategyStudyStage.VALIDATE
    ...
    def stage_started(self, stage: StrategyStudyStage) -> None:
    ...
    def substage_progress(
        ...
        *,
        ...
        if name != self._current_substage:
            raise ValueError("strategy study progress substage is not active")
        if item_total <= 0 or not 1 <= item_completed <= item_total:
            raise ValueError("strategy study item progress is out of bounds")
    ...
    def _substage_event(
        ...
        if substage is None:
            raise ValueError("strategy study progress has no active substage")
        context: dict[str, JsonValue] = {
            "substage": substage,
    ...
    @staticmethod
    def _stage_index(stage: StrategyStudyStage) -> int:
        try:
            return STRATEGY_STUDY_STAGES.index(stage)
        except ValueError as error:
            raise ValueError(f"unknown strategy study stage: {stage}") from error

    @staticmethod
    def _substage(value: str) -> str:
        if not isinstance(value, str) or not value.strip():
            raise ValueError("strategy study substage must be nonempty")
        return value


__all__ = ["StrategyStudyProgressReporter"]
