...
异常：
    输入违反领域不变量时抛出类型或值错误；依赖失败保持原异常语义。
        """
        item = next((value for value in self.estimates if value.instrument_id == instrument_id), None)
        if item is None or weight_change == 0.0:
            return 0.0
        size = abs(weight_change)
        return item.fixed_cost + item.linear_coefficient * size + item.impact_coefficient * sqrt(size)
...
异常：
    ...
    def __init__(self, *, fixed_bps: float, impact_bps: float, max_participation: float) -> None:
    ...
    def build(self, signal_date: date, liquidity: dict[str, float], portfolio_value: float) -> PreTradeCostSlice:
        """按证券 ADV 生成权重容量和冲击参数。
    ...
    输入违反领域不变量时抛出类型或值错误；依赖失败保持原异常语义。
        ...
        estimates = tuple(
            CostEstimate(
                ...
                0.0,
                self._impact,
                max(0.0, amount * self._participation / portfolio_value) if portfolio_value > 0.0 else 0.0,
            )
...
异常：
    ...
    def __init__(self, *, commission_bps: float, sell_tax_bps: float, slippage_bps: float) -> None:
...
异常：
    输入违反领域不变量时抛出类型或值错误；依赖失败保持原异常语义。
        ...
        return RealizedCost(round(amount * self._commission), round(amount * self._tax) if is_sell else 0, round(amount * self._slippage))
