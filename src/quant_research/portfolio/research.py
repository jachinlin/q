...
class AlphaRiskCostOptimizer:
    ...
    def __init__(
        ...
        *,
        ...
        if any(
            ...
            for value in (risk_aversion, cost_aversion)
        ):
    ...
    def construct(
        ...
        *,
        ...
        candidates = [
            item for item in signals if item.is_valid and item.score is not None
        ]
        candidates.sort(key=lambda item: (-cast(float, item.score), item.instrument_id))
        ...
        risk_index = {
            instrument: index for index, instrument in enumerate(risk.instruments)
        }
        ...
        covariance: NDArray[np.float64] = np.asarray(risk.covariance, dtype=np.float64)[
        ...
        for _ in range(self._iterations):
            ...
            next_weights = self._project(
                weights + self._learning_rate * gradient, instruments, costs
            )
        ...
        cost_penalty = float(
            self._cost
            * sum(
                costs.estimate(instrument, float(weight - current[index]))
                for index, (instrument, weight) in enumerate(
                    zip(instruments, weights, strict=True)
                )
            )
        )
        positions = tuple(
            TargetPosition(
                ...
                float(weight),
                cast(float, candidates[index].score),
                "ALPHA_RISK_COST_SELECTED",
            )
            for index, (instrument, weight) in enumerate(
                zip(instruments, weights, strict=True)
            )
        ...
        return ConstructedPortfolio(
            ...
            ObjectiveBreakdown(
                alpha, risk_penalty, cost_penalty, alpha - risk_penalty - cost_penalty
            ),
        )
...
class DirectionalExposureMapper:
    ...
    def construct(
        self, signal: DirectionalSignalRow, execute_date: date
    ) -> TargetPortfolio:
        """拒绝 SHORT，并返回单资产目标组合。
        ...
        positions = (
            ...
            if weight == 0.0
                ...
                TargetPosition(
                    InstrumentId.parse(signal.instrument_id),
                    weight,
                    signal.strength,
...
class AllocationProjector:
    ...
    def construct(
        self, signals: tuple[AllocationSignalRow, ...], execute_date: date
    ) -> TargetPortfolio:
        """把非负目标暴露投影到多头权重单纯形。
        ...
        valid = [
            ...
            for item in signals
            if item.is_valid
            and item.desired_exposure is not None
            and item.desired_exposure > 0.0
        ]
        if not signals:
        ...
        raw = (
            [(item, min(self._max_weight, value / total)) for item, value in exposures]
            if total > 0.0
            else []
        )
        positions = tuple(
            TargetPosition(
                ...
                weight,
                cast(float, item.desired_exposure),
                "ALLOCATION_PROJECTED",
            )
            for item, weight in sorted(raw, key=lambda pair: pair[0].instrument_id)
        )
        cash = round(max(0.0, 1.0 - sum(item.target_weight for item in positions)), 12)
