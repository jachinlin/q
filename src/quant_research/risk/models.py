...
class RiskSlice:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            len(self.annualized_volatility) != size
            or len(self.liquidity_amount) != size
        ):
...
class StatisticalRiskEstimator:
    ...
    def estimate(
        self, observations: pl.DataFrame, decision_dates: tuple[date, ...]
    ) -> RiskArtifact:
        ...
        for decision_date in sorted(set(decision_dates)):
            history = (
                observations.filter(pl.col("trade_date") <= decision_date)
                .sort("trade_date")
                .tail(self._lookback * observations["instrument_id"].n_unique())
            )
            pivot = history.pivot(
                on="instrument_id",
                index="trade_date",
            ...
            instruments = tuple(
                sorted(column for column in pivot.columns if column != "trade_date")
            )
            if not instruments:
                continue
            ...
            volatility = tuple(
                float(sqrt(max(covariance[index, index], 0.0) * 252.0))
                for index in range(len(instruments))
            )
            liquidity_rows = (
                history.group_by("instrument_id")
                .agg(pl.col("amount").mean())
                .to_dicts()
            )
            liquidity_map = {
                str(row["instrument_id"]): float(row["amount"] or 0.0)
            ...
            slices.append(
                RiskSlice(
                    ...
                    tuple(liquidity_map.get(item, 0.0) for item in instruments),
                )
            )
