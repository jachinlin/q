...
独立 FactorStudy 已实现该显式消费契约：`industry: null` 只发布 `DIRECTION_ADJUSTED`；配置行业块后，同一研究同时发布 `DIRECTION_ADJUSTED` 与 `INDUSTRY_NEUTRALIZED`。中性化固定使用信号日 `industry_code` 的等权组内去均值，状态通过一次 `industry_classifications_on_dates` 批量读取；`EXCLUDE` 排除 tombstone 和缺失状态，`UNCLASSIFIED` 将二者放入固定未分类组。逐日分类、tombstone、缺失、可用数量和覆盖率写入 `industry_coverage.parquet` 和研究 Manifest。股票策略、组合约束和回测归因仍未启用行业消费，不能仅凭研究 Manifest 中存在行业输入就推断策略使用了行业信号。
