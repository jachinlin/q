...
- 基准行情按完整回测交易日区间一次读取并验证覆盖、唯一性和正有限价格，逐日估值只做
  日期索引，不得重复扫描同一指数数据集。
- 回测通过消费者侧 `BacktestCorporateActionData` 显式注入权益事件，并在交易日循环前
  一次预载研究区间。登记日成交后按收盘持仓锁权；除权日开盘前确认税前现金应收和
  FLOOR 后的送转股份；支付日映射到不早于官方日期的首个交易会话并在撮合前转为现金。
  ETF 份额拆分由基金复权因子的唯一近整数倍率驱动（相对误差上限 0.1%，原始倍率与
  误差写入审计），在生效会话开盘前增加份额；非整数折算失败关闭。送转和拆分后按
  股份数量比例确定性重分配整数分成本，余数归稳定排序的最后一批，账户总成本不变。
- 新成交证券由待执行委托进入当日行情范围；停牌持仓使用账户最近有效估值价；范围内
  缺失行情继续产生 `NO_MARKET_DATA`，不得改变撮合、估值、PIT 或目录漂移语义。
- Canonical 内容寻址分区在进程内首次读取时验证路径、Schema、行数和 SHA-256，后续读取
  复用物理分区租约；外层研究阶段前后检查 `catalog_hash`，回测引擎自身只在开始和结果
  生成前检查，不在交易日循环内重复查询目录身份。
...
- `monthly_returns`、`annual_returns`、`exposure_summary`、`attribution` 和
  `execution_summary`：分别支持周期收益、证券/现金/分红应收暴露、证券归因和成交质量分析；
- `dividends`：逐项记录 `CASH_DIVIDEND`、`STOCK_DISTRIBUTION` 或 `FUND_SPLIT`，包含
  确定性事件 ID、来源 revision、官方与映射日期、登记数量、税前现金、应收/实付、
  送转或拆分比例、新增数量、舍弃小数，以及复权因子审计证据；
...
核心指标和完整策略/毛值/基准净值，再展示回撤、252 日滚动指标、月度热力图、年度收益、
 证券/现金/分红应收暴露、成本拖累、成交质量、证券归因和主要回撤事件。完整指标按收益、风险、基准
