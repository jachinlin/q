...
三张财务报表使用 VIP 全市场端点，并显式传递 `report_type=1`，只保存上市公司合并报表。股票分红按 `ann_date`、`imp_ann_date` 两种事件日期切片；基金分红按 `ann_date`、`ex_date`、`pay_date` 三种日期切片。不同切片可能返回同一业务记录，CURATE 会按业务内容去重。基金端点同时返回场内基金和以 `.OF` 结尾的场外基金；Raw 保留完整供应商响应，行情、财务和分红 Canonical 的场内证券代码必须严格符合六位数字加 `.SH`、`.SZ` 或 `.BJ`。
...
Canonical 发布不参与并行：文件写入与校验、SQLite 当前指针切换、目录身份更新和阶段
状态登记共用一把发布锁。这样既让耗时的 Parquet/Polars 计算并行，又避免多个数据集在
SQLite 写事务上发生竞态。任一数据集失败时，尚未开始的工作被取消，运行中的线程在
Raw 或分区安全边界退出；已经原子发布的数据集保留为可复用检查点。

Canonical 文件按内容寻址且发布后不可变。当前指针切换后，在线发布流程不会立刻删除旧
文件，因为调用方可能已经拿到指向旧内容的 `LazyFrame`，但尚未执行 `.collect()`。旧文件
不在 SQLite 当前目录中，因而不能作为历史 Snapshot 被新查询发现；它只用于保证已返回
查询计划的执行安全。若以后增加空间回收，必须在排空研究读取者的独占维护窗口中完成。
...
`pit_usable=false` 表示该行保留作供应商事实和审计证据，但研究读取不能把它当成时点安全观测；此时 `available_at` 可以为空。财务公告早于报告期、报告期不是合法季末或报表口径不符合合并报表契约时，Mapper 会明确把该行降为不可用于 PIT，而不是伪造可用时间。
...
Canonical 文件成功写出不代表数据一定正确。VALIDATE 会先按 SQLite 当前目录重新核对每个
分区的内容哈希、Schema 指纹、行数和可信路径；任一物理身份不一致都会直接
失败，不能仅凭“还能被 Parquet 读取且业务值看起来合理”开放研究门禁。随后检查数据契约和
常见数据问题，例如：

- 必需数据集或分区是否缺失、是否为空；
- Schema、列顺序和类型是否符合声明；
- 主键是否重复，关键字段是否为空；
- 所有 `instrument_id` 是否为六位 ASCII 数字并使用 `.SH`、`.SZ` 或 `.BJ` 后缀；
- 对成交量或成交额非零的真实成交行，OHLC 是否满足 `high ≥ open/close ≥ low`；
- 对真实成交行，价格是否为正且为有限数；零成交量、零成交额占位行不冒充成交；
- 成交量是否为负；
- `pct_change` 是否与 `close / preclose - 1` 在供应商价格精度允许的 `0.001` 绝对误差内一致；
...
一次质量运行按数据集顺序处理，每个数据集的物理分区只物化一次；同一数据集的 Schema、
主键、空值、数值与 PIT 规则复用内存中的结果，跨数据集覆盖规则只保留去重后的必要列。
这把磁盘扫描次数固定在数据集级，同时避免把全部数据集一起常驻内存。
...
分红 Canonical 使用公告日与实施公告日中较晚者作为该条信息的 `available_at`，避免供应商历史修订中异常偏早的实施公告日期造成前视。预案、取消或尚未实施的记录可以没有登记、除息或支付日期，不能因此伪造日期。
...
- 交易日历需要覆盖未来规划窗口；
- 财务数据按披露截止日判断是否应该更新；
- 分红按北京时间 18:00 切换完整自然日：此前更新到前一天，此后可以覆盖当天；新鲜度使用成功 LOCALIZE 的 `localized_through`，当天无分红记录也不会误判为陈旧；
- 快照类数据按最近成功刷新时间判断。
...
策略研究或因子研究提交时捕获当前 `catalog_hash`。运行阶段前后都会检查它：
...
`.collect()` 时才读取 Parquet。调用方后续追加的过滤和投影也能继续参与 Polars 优化，避免
先由 Repository 把 DuckDB/Arrow 结果全部物化后再伪装成惰性结果。
...
策略研究配置中的 benchmark 在解析时转换为 `IndexId`。回测通过 `index_bars()` 单独读取指数收盘价，不把指数插入可交易 MarketSlice，也不允许进入订单或持仓。
...
| 策略研究提示数据漂移 | 运行期间 `catalog_hash` 变化 | 数据更新并重新验证后复制配置并提交新研究 |
