...
报告期的最新修订；若该最新记录的目标字段无效，不回退到旧报告。修订历史按证券、可用时间和
报告期在 Polars 中向量化生成状态转换，不得把全市场财务历史物化为 Python 行对象；同一可用时间
的多条修订先确定各报告期最新版本，再选择截至该事件的最新报告期，保持原有 PIT 与不回退语义。
...
因子研究是独立 `FactorStudy`，不属于策略研究，也不共享策略研究任务或产物。
